...
def _extract_tool_calls(resp) -> List[Any]:
    """Devuelve la lista de tool_calls desde required_action; soporta dict/obj."""
...
#
# Recolector de texto robusto
def _collect_text(resp) -> str:
    # 1) si output_text existe, úsalo
...
# Detectar si el texto parece intención Git (para fallback)
...
def _try_git_fallback(user_msg: str) -> Optional[str]:
    ...
    try:
        ...
        if not intent:
            return None
        # Usa tus acciones ya probadas con allowed_dirs
...
# ------------------------------------------------------------
# Servicio de chat con tool-calling (Responses API)
# ------------------------------------------------------------
class ToolCallingChatService:
        ...
        while getattr(resp, "status", None) == "requires_action" and guard < 8:
            ...
            for tc in tcalls:
                ...
                if name and name.startswith("git_"):
                    ...
                    # Si no estaba inicializado, init + retry
    ...
    def _drain_or_drop(self, prev_id: Optional[str]) -> Optional[str]:
        if not prev_id:
            return None
        try:
            r = self.llm.client.responses.retrieve(prev_id)  # *** USAR retrieve() ***
    ...
    def ask(self, session_id: str, user_msg: str, max_output_tokens: int = 700) -> str:
        ...
        git_intent_text = _try_git_fallback(user_msg) #sssssssss
        if git_intent_text:
            # no guardes prev_id; esto es ejecución directa
            self.sessions[session_id] = None
